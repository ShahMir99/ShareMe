...
export const getAllPost = async (req, res) => {
  try {
    ...
    const userposts = await POST.find({ user : req.params.id}).populate("user Comments.user")
    ...
    res.status(201).json([...AllPost.concat(userposts).sort((a, b) => b.createdAt - a.createdAt)]);
  } catch (err) {
    res.status(500).json(err);
  }
};
